...
class Kuramoto:
    ...
    def __init__(self, coupling=1, dt=0.01, T=10, n_nodes=None, natfreqs=None, forcing_amp=0, forcing_freq=0):
        '''
        coupling: float
        ...
        forcing_frequency: float 
            Default = 0 
            Frequancy of the external force
        '''
        ...
        if forcing_amp == 0:
            warnings.warn("If the forcing frequency is zero, so it is the amplitude", Warning)
            self.forcing_freq = 0            
    ...
    def derivative(self, angles_vec, t, adj_mat, coupling):
        ...
        dxdt = (self.natfreqs - self.forcing_freq) + coupling * interactions.sum(axis=0) - self.forcing_amp*np.sin(angles_vec)
    ...
    def mean_frequency(self, act_mat, adj_mat):
        ...
        for time in range(n_steps):
            dxdt[:, time] = self.derivative(act_mat[:, time], None, adj_mat, self.coupling)
